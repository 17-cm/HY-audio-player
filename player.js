...
(function() {
    ...
    const MusicPlayerApp = {
        ...
        startRainbowTimer() {
            setInterval(() => {
                if (this.state.rgbMode === 2) {
                    ...
                    if (root) root.style.setProperty('--rainbow-offset', hueOffset + 'deg');
                    if (rhythmIcon) rhythmIcon.style.setProperty('--rainbow-offset', hueOffset + 'deg');
                }
            }, 2000);
        },
        ...
        play(i) {
            ...
            // 解析歌词
            if (this.playlist[i].lyrics) {
                this.state.lyrics = this.parseLyrics(this.playlist[i].lyrics);
            } else {
                this.state.lyrics = [];
            }
            ...
            this.fetchNeteaseSongInfo(input)
                .then(songInfo => {
                    const newSong = {
                        ...
                        cover: defaultConfig.cover // 使用默认封面，不使用歌曲封面
                    };
                ...
                for (let i = 0; i < playlist.tracks.length; i++) {
                    ...
                    try {
                        ...
                        const newSong = {
                            ...
                            url: songInfo.url,
                            lyrics: songInfo.lyrics || '',
                            cover: defaultConfig.cover // 使用默认封面
                        };
        ...
        injectCSS() {
            const css = `
                ...
                /* 幻彩色相变化 */
                @keyframes hue-shift {
                    0% { filter: hue-rotate(0deg) saturate(0.9) brightness(1.05); }
                    25% { filter: hue-rotate(90deg) saturate(1) brightness(1); }
                    50% { filter: hue-rotate(180deg) saturate(0.85) brightness(1.1); }
                    75% { filter: hue-rotate(270deg) saturate(0.95) brightness(1.05); }
                    100% { filter: hue-rotate(360deg) saturate(0.9) brightness(1.05); }
                }
                ...
                .rhythm-star.star-left {
                    left: 35px;
                    animation: star-left 1.3s ease-in-out infinite;
                }
                
                .rhythm-star.star-right {
                    right: 35px;
                    animation: star-right 1.9s ease-in-out infinite;
                }
                
                /* 星星单色RGB */
                .player-rhythm-icon.rgb-single .rhythm-star {
                    color: var(--rgb-single);
                    text-shadow: 0 0 10px var(--rgb-single), 0 0 20px var(--rgb-single);
                }
                ...
                .player-rhythm-icon.rgb-rainbow .rhythm-star {
                    ...
                    animation: star-left 1.3s ease-in-out infinite, hue-shift 4s linear infinite;
                }
                
                .player-rhythm-icon.rgb-rainbow .rhythm-star.star-right {
                    animation: star-right 1.9s ease-in-out infinite, hue-shift 4s linear infinite 1s;
                }
                ...
                .player-rhythm-icon.rgb-single .rhythm-bar { 
                    background: linear-gradient(to top, var(--rgb-single), color-mix(in srgb, var(--rgb-single) 60%, white));
                ...
                .player-rhythm-icon.rgb-rainbow .rhythm-bar { 
                    background: linear-gradient(to top, #ff6b6b, #ffd93d, #6bcb77, #4d96ff);
                    box-shadow: 0 0 5px rgba(255,100,100,0.5);
                    animation: wave var(--s) infinite ease-in-out alternate, hue-shift 3s linear infinite;
                ...
                .player-rgb-border.mode-single::before {
                    ...
                    background: conic-gradient(
                        from 0deg,
                        transparent 0deg,
                        color-mix(in srgb, var(--rgb-single) 40%, white) 60deg,
                        var(--rgb-single) 120deg,
                        color-mix(in srgb, var(--rgb-single) 70%, white) 180deg,
                        var(--rgb-single) 240deg,
                        color-mix(in srgb, var(--rgb-single) 40%, white) 300deg,
                        transparent 360deg
                    );
                ...
                .player-rgb-border.mode-rainbow::before {
                    ...
                    animation: rgb-rotate 4s linear infinite, hue-shift 6s linear infinite;
                }
                ...
                .player-island.rgb-single {
                    ...
                    background-size: 200% 100%;
                    animation: island-flow 2s linear infinite, breathe 2s ease-in-out infinite;
                    box-shadow: 0 0 15px var(--rgb-single);
                }
                ...
                .player-island.rgb-rainbow {
                    ...
                    animation: island-flow 3s linear infinite, hue-shift 5s linear infinite;
                ...
                .player-lyrics { 
                    ...
                    overflow: hidden; 
                    text-align: center;
                    background: linear-gradient(90deg, var(--lyrics-start), var(--lyrics-end));
